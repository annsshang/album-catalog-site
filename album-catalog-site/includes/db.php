...
{
  // Resolve relative paths to absolute paths (based off project root = parent of includes/)
  $project_root = realpath(__DIR__ . '/../');
  if ($project_root === false) {
    $project_root = __DIR__ . '/../';
  }

  // If a path is not absolute, make it relative to project root so behavior is deterministic
  if (strpos($db_filename, DIRECTORY_SEPARATOR) !== 0 && !preg_match('/^[A-Za-z]:\\\\/', $db_filename)) {
    $db_filename = $project_root . DIRECTORY_SEPARATOR . ltrim($db_filename, DIRECTORY_SEPARATOR);
  }
  if (strpos($init_sql_filename, DIRECTORY_SEPARATOR) !== 0 && !preg_match('/^[A-Za-z]:\\\\/', $init_sql_filename)) {
    $init_sql_filename = $project_root . DIRECTORY_SEPARATOR . ltrim($init_sql_filename, DIRECTORY_SEPARATOR);
  }

  // Diagnostic log to show exactly which files are being used
  error_log("init_sqlite_db: resolved db_filename={$db_filename}, init_sql_filename={$init_sql_filename}");
